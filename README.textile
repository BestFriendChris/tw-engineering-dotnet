...
h3. Customer and its Tests

CustomerTests has some really badly named tests:

        public void NonEmpty()
        public void Empty()

This is a good place to discuss DDD :-D

Note that the statement does not contain the number of days for a rental. 
This is in the original version of the code but is probably a Bad Thing.

h3. Repositories using Specification
Not necessarily the most straight forward way, tried to emulate the Java version, maybe a good place to discuss various approaches.

CustomerRepository tests do not include all flows 

h3.
Feature class
CurrentIteration(). Refactor? yes ! 